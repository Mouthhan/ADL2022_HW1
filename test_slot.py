...
def main(args):
    ...
    model.eval()
    model.load_state_dict(torch.load('./ckpt/slot/best.ckpt'))
    result_dict = {}
    out_count = 0  
    model.eval()  # set the model to evaluation mode
    with torch.no_grad():
        for i, data in enumerate(test_loader):
            inputs, index = data['tokens'], data['id']
            inputs = inputs.to(device)
            outputs = model(inputs)
            test_pred_list = []  
            for idx in range(outputs.shape[0]):
                seq_tag = []
                _, test_pred = torch.max(outputs[idx], 1)
                for tag in test_pred:
                    if len(seq_tag)< len_list[out_count]:
                        seq_tag.append(tag.item())
                    else:
                        break
                out_count += 1
                test_pred_list.append(seq_tag)
            for y in range(len(test_pred_list)):
                tag_list = ""
                for tag in range(len(test_pred_list[y])):
                    tag_list += SeqClsDataset.idx2label(
                        dataset, idx=test_pred_list[y][tag])
                    if tag != len(test_pred_list[y]) - 1:
                        tag_list += " "
                result_dict[index[y]] = tag_list
